...
-- 5) Скорость доставки (дни) по штатам
SELECT c.customer_state,
...
-- 6) Средняя оценка по категориям (>=50 отзывов)
SELECT COALESCE(t.product_category_name_english,'(unknown)') AS category,
...
-- 9) Топ товаров по количеству проданных позиций
